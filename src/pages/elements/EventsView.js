...
function EventsView(eventData) {
  ...
  const [activeEvents, setActiveEvents] = useState(
    eventData.eventData.events.filter((event) => event.eventStatus === "active")
  );
  const [closedEvents, setClosedEvents] = useState(
    eventData.eventData.events.filter((event) => event.eventStatus === "closed")
  );
  const [inactiveEvents, setInactiveEvents] = useState(
    eventData.eventData.events.filter(
      (event) => event.eventStatus === "inactive"
    )
  );
  ...
  if (meError)
    return <div className={Styles.error}>Inicie sesión para continuar</div>;
  if (meData.me.userType !== "admin")
    return (
      <div className={Styles.error}>
  ...
  const clickEvent = (eventTrip, eventDate) => {
    Router.push({
      pathname: "/elements/EventDetails",
      query: { eventTrip, eventDate },
    });
  };
    ...
    switch (e.target.name) {
      ...
      case "closedEvents":
        ...
        searchBar(
          e.target.value,
          eventData.eventData.events.filter(
            (event) => event.eventStatus === "closed"
          ),
          setClosedEvents
        );
        break;
        ...
        <div className={Styles.contenedorEvents} key={i++}>
          {activeEvents?.map((event) => (
            <div
              ...
              key={event.eventTrip + event.eventDate}
            >
              ...
              <div className={Styles.miniFlex}>
                <div className={Styles.textHid}>Tipo de viaje:&nbsp;</div>{" "}
                {event.eventType === "Public" ? "Público" : "VIP"}
              </div>
              <button
                className={Styles.btn}
                onClick={() => {
                  clickEvent(event.eventTrip, event.eventDate);
                }}
              >
                Ver más
              </button>
